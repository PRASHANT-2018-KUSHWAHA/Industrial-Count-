...
import { initialInventory, initialTimeline, skuCountOptions, SkuCountOption } from './data';
import LoginScreen from './components/LoginScreen';
import DashboardScreen from './components/DashboardScreen';
import ReceiveScreen from './components/ReceiveScreen';
import TimelineScreen from './components/TimelineScreen';
import ProfileScreen from './components/ProfileScreen';
import { Bell, LayoutDashboard, History, PlusSquare, UserCircle, CheckCircle, Flame, LogOut } from 'lucide-react';
...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'timeline' | 'receive' | 'profile'>('overview');
  const [selectedSkuToReceive, setSelectedSkuToReceive] = useState<string | undefined>(undefined);
  ...
  const handleCreateCustomSku = (newOpt: SkuCountOption) => {
    setCustomSkus(prev => {
      ...
      if (prev.some(x => x.sku.toUpperCase() === newOpt.sku.toUpperCase() || x.sku.toUpperCase() === 'SKU-' + newOpt.sku.toUpperCase())) {
        return prev;
      }
      return [...prev, newOpt];
    });
    ...
    setInventory(prev => {
      ...
      if (prev.some(x => x.sku.toUpperCase() === formattedSku.toUpperCase())) {
        return prev;
      }
  ...
  const handleConfirmReceipt = (sku: string, verifiedCount: number, serial: string) => {
    ...
    const timestampStr = rightNow.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    ...
    const freshEvent: TimelineItem = {
      ...
      reference: serial ? `Serial: ${serial}` : `Inspected: ${user?.name || 'Operator'}`,
    };
    ...
    setActiveTab('overview');
    triggerNotification(`SUCCESS: Received +${verifiedCount} ${itemName} items!`);
  };
  ...
  const handleResetDatabase = () => {
    ...
    triggerNotification('Plant database successfully reset to factory parameters.');
  };
  ...
  const shiftInspectionsCount = timeline.filter(t => t.id.startsWith('tx-user-')).length;
    ...
    <div className="min-h-screen bg-slate-100 flex justify-center items-stretch antialiased font-sans">
      ...
      <div className="w-full max-w-md bg-[#f7f9fb] flex flex-col items-stretch relative shadow-2xl border-x border-[#bdc9c7]">
        ...
        <header className="fixed top-0 max-w-md w-full shrink-0 z-40 bg-white border-b border-[#bdc9c7]/30 flex justify-between items-center h-14 px-4 shadow-sm">
          ...
          <button 
            onClick={() => triggerNotification('Current Plant communication channels are operate nominally.')}
